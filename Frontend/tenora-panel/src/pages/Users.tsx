...
interface UserItem {
  id: number; email: string; is_admin: boolean; created_at: string;
...
export default function Users() {
  ...
  return (
    <div className="space-y-6 animate-fade-up">
      <PageHeader eyebrow="Administration" title="Utilisateurs" subtitle={`// ${total} compte(s)`} />

      <DataCard>
        <DataCardHeader>
          <div className="relative flex-1 max-w-xs">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
            <Input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder="Rechercher..." className="pl-9 h-9 rounded-none border-2 mono text-xs" />
          </div>
          <span className="chip border-border ml-auto"><UsersIcon className="h-3 w-3" /> {total}</span>
        </DataCardHeader>
        <DataCardContent>
          {loading ? (
              ...
              {users.map((u) => (
                <div key={u.id} className="flex items-center gap-3 p-3 sm:p-4 hover:bg-muted/30 transition-colors">
                  <div className={cn(
                    ...
                    u.is_admin ? "border-secondary bg-secondary text-secondary-foreground" : "border-primary/40 bg-primary-soft text-primary"
                  )}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] mono text-muted-foreground">{fmtDate(u.created_at)}</p>
                  </div>
